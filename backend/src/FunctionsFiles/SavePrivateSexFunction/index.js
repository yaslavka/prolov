...
async function SavePrivateSexFunction(req) {
  let message;
  try {
    ...
    if (!authorization) {
      ...
      if (!userAuth) {
        ...
        const { privateSex } = req;
        await UserTable.update({ privateSex }, { where: { id: userAuth.id } });
        const newArrUsers = alUsers.filter((us) => us.id !== userAuth.id);
        const user = { ...userAuth, privateSex: privateSex };
